...
impl BinaryBuf {
    ...
    fn read_int(&self, start_bit: usize, end_bit: usize) -> usize {
        let mut r: usize = 0;               // return value
...
fn main() {
    ...
    loop {
        let p = read_packet(&mut in_bin, &mut cur_idx, &mut packets, usize::MAX);
        
        if (in_bin.data.len() * 8) - cur_idx < 11  {
            //println!("Done - cur_idx: {}, in_bin.data.len() in bits: {}", cur_idx, in_bin.data.len() * 8);
            break;
        }
    }
    ...
    fn read_packet(in_bin: &mut BinaryBuf, cur_idx: &mut usize, packets: &mut Vec<Packet>, parent_idx: usize) -> (usize, usize, usize) {
        ...
        if packet_type == 4 {
            ...
            //println!("literal {} - packet_start: {}, cur_idx: {}, my_idx: {}, parent_idx: {:?}", literal_val, packet_start, *cur_idx, my_idx, packets[my_idx].parent);            
        }
        else {
            ...
            *cur_idx = packet_start + 7 + total_length_bits;
            ...
            //println!("operator - packet_start: {}, cur_idx: {}, my_idx: {}, parent_idx: {:?}", packet_start, *cur_idx, my_idx, packets[my_idx].parent);
            //println!("{:?}", packets[my_idx]);

            

            while (length_type_id == 0 && (sub_bits_read < total_length)) || (length_type_id == 1 && (sub_packets_read < total_length)) {
                ...
                sub_bits_read += pr.1;
                sub_packets_read += pr.2;                
            }            
        }

        if packets[my_idx].parent.is_some() {
            sub_packets_read += 1;
            sub_bits_read += *cur_idx - packet_start;
        }

        (my_idx, sub_bits_read, sub_packets_read)
    }

    // Sum up each packet version
    let mut version_total: usize = 0;
    for i in 0..packets.len() {
        version_total += packets[i].version;
    }
    println!("Total of all packet versions: {}", version_total);
}
