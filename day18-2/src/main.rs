...
fn main() {
    ...
    let mut sums: Vec<u32> = Vec::with_capacity(20_000);
    ...
    for i in 0..nums.len() {
        for j in 0..nums.len() {
            ...
            let mut vnew_i: Vec<u32> = nums[i].clone();
            let mut vnew_j: Vec<u32> = nums[j].clone();

            add_encoded(&mut vnew_i, &nums[j]);
            add_encoded(&mut vnew_j, &nums[i]);

            reduce(&mut vnew_i);
            reduce(&mut vnew_j);

            magnitude(&mut vnew_i);
            let m_i = vnew_i[0];

            magnitude(&mut vnew_j);
            let m_j = vnew_j[0];
