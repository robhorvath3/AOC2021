...
fn main() {
    ...
    let limit_cube: Cuboid = Cuboid { is_on: false, minval: [-50, -50, -50], maxval: [50, 50, 50] };

    let mut newcubes: Vec<Cuboid> = Vec::with_capacity(1024);

    for i in 0..cuboids.len() {
        let lcube = cuboids[i].intersect(&limit_cube);
        
        if lcube.is_none() {
            continue;
        }
        
        let mut lcube = lcube.unwrap();
        lcube.is_on = !lcube.is_on;

        let mut mergecubes: Vec<Cuboid> = Vec::with_capacity(128);

        if lcube.is_on {
            mergecubes.push(lcube);
        }

        for j in 0..newcubes.len() {
            let intersect = newcubes[j].intersect(&lcube);
