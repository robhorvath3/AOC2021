        ...
        for i in 0..boards.len() {
            if boards[i].col[x] != 9999 {
                ...
                for r in 0..boards[i].score.row.len() {                        
                    if boards[i].score.row[r] == 0b11111 {
                        ...
                        println!("Winner on board {:?}, the unmarked sum is {}, the number just called was {} and the factor is {}", boards[i], unmarked_sum, x, unmarked_sum * x as u32);
                        panic!("Winner Found!");
                    }
                }

                for c in 0..boards[i].score.col.len() {
                    if boards[i].score.col[c] == 0b11111 {
